...
Matrix createLocalK(int e,mesh &m){
    ...
    float t,k,g,h,Ae,Ae2,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixM,matrixD;
    Matrix K,g_matrix,g_matrix_t,Alpha,Beta,Alphat,Betat,Alpha2,Beta2,Alphat2,Betat2,BPrima,BPrimat;
    ...
    productRealMatrix(J/(6*h*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);

    //Preparando matrixM (En clase conocida simplemente como M)
    g = -m.getParameter(G);
    Ae2 = calculateLocalArea(e,m);
    productRealMatrix(g*Ae2/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixM);
...
Vector createLocalb(int e,mesh &m){
    ...
    b.push_back(J*n/6); b.push_back(J*n/6); b.push_back(J*n/6);
